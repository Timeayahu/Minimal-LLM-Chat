...
from tools.planner import decide_agent_step
...
def _run_agent_loop(
    ...
    for step in range(start_step, MAX_AGENT_STEPS + 1):
        ...
        plan = decide_agent_step(state.user_input, tools, state.observations)
...
def _handle_final_action(
    ...
    if (
        ...
        handle_chat_message(session, state.user_input)
        return
